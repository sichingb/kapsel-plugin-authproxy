 ...
 */
AuthProxy.prototype.setAutoSelectCertificateConfig = function (successCB, errorCB, enableAutoSelectSingleCert) {
    if (cordova.require("cordova/platform").id == "ios"){
        cordova.exec(successCB, errorCB, "AuthProxy", "setAutoSelectCertificateConfig", [enableAutoSelectSingleCert]);
    }
    else{
        successCB();
    }
...
//SAML config parameter samlConfig is a js object, currently the only required property is
//saml2.web.post.authchallengeheader.name
//   {
//    saml2.web.post.authchallengeheader.name: "com.sap.cloud.security.login",
//   }

//callback parameter smalChallengeCallback takes a parameter containing
//the request url

//To remove and disable SAML challenge handler, js side can calls the method by setting samlConfig to null,

var _samlConfig = null;
var _smalChallengeCallback = null;
...
AuthProxy.prototype.setSAMLChallengeHandler = function(successCallback, errorCallback, samlChallengeCallback, samlConfig) {
    ...
    // samlConfig required
    if ( samlConfig && !(samlConfig["saml2.web.post.authchallengeheader.name"])) {
        console.log("AuthProxy Error: samlconfig does not specify saml2.web.post.authchallengeheader.name setting");
        errorCallback({
            errorCode: AuthProxy.prototype.ERR_MISSING_PARAMETER,
            description: AuthProxy.prototype.MSG_MISSING_PARAMETER + "samlConfig"
        });
        return;
    }
    
    _samlConfig = samlConfig;
    _smalChallengeCallback = samlChallengeCallback;
    
    //override the xhr onload event listener to check the SAML challenge header,
    //note the onload event listener is per xhr request, and also it may not be set
    //until send method is called. so we cannot override the
    //xmlhttprequest.prototype.onload to replace it. Instead we override send method,
    //as when send method is called, the onload event should already be set and is
    //available to be overriden
    if (_originalSend == null){
        _originalSend = window.XMLHttpRequest.prototype.send;
    
        window.XMLHttpRequest.prototype.send = function () {
            
            if (_samlConfig != null){
                var checkSAMLChallengeAtOnLoad = function(originalOnLoadListener){
                    this.handler = function(){
                        //check response for saml auth header, if so call saml challenge handler
                        var header = this.getResponseHeader(_samlConfig["saml2.web.post.authchallengeheader.name"]);
                        if (header != null){
                            _smalChallengeCallback(this["url"]);
                        }
                        //if original listener is not null, then invoke it
                        if (originalOnLoadListener){
                            originalOnLoadListener.apply(this, arguments);
                        }
                    }
                };
                
                var originalOnLoad = this["onload"];
                this["onload"] = (new checkSAMLChallengeAtOnLoad(originalOnLoad)).handler;
            }
            _originalSend.apply(this, arguments);
        }
        
        var _originalOpen = window.XMLHttpRequest.prototype.open;
...
var setupXhrHttpsAutoConversionForAndroid = function() {
    ...
    sap.AuthProxy.isInterceptingRequests( function(isIntercepting) {
        if (!openModified && isIntercepting) {
                ...
                if (arguments.length >= 2){
                    ...
                    if (url.toLowerCase().indexOf("https") == 0) {
                        ...
                        if (!sap.AuthProxy.isHttpsConversionHost(url)) {
                            sap.AuthProxy.addHTTPSConversionHost(function() {},function(){}, url);
                        }
                    }
                }
...
document.addEventListener("deviceready", function() {
    if (device.platform.toLowerCase().indexOf("android") >= 0) {
        ...
        setupXhrHttpsAutoConversionForAndroid();
    }
    else if (cordova.require("cordova/platform").id == "ios"){
        (function() {
            ...
            window.XMLHttpRequest.prototype.open = function (method, url, async, user, password) {
                ...
                if (user != null && password != null) {
                    this.setRequestHeader("XhrOpenWithCredential", "true");
                }
            };
         })();
    }
});
// Need setup xhr https conversion on this event too, since AuthProxy will not
// be intercepting requests on the deviceready event if the Logon plugin is present.
document.addEventListener("onSapLogonSuccess", setupXhrHttpsAutoConversionForAndroid);
